...
5. AI 활용 규칙 (나만의 코파일럿)
코딩을 시작하기 전, 작성해둔 [요구사항 템플릿]을 활용해 AI에게 전체적인 구조를 먼저 물어본다.

에러가 났을 때는 당황하지 않고 [디버깅 템플릿]에 에러 메시지와 내 코드를 넣어 질문한다.

구현을 완료한 후에는 [코드 리뷰 템플릿]을 통해 더 나은(효율적인) 코드가 있는지 피드백을 받는다.
